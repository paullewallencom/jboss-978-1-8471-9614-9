...
public class ChildHome {
	...
	public Child merge(Child detachedInstance) {
		...
		try {
			Child result = (Child) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Child findById(int id) {
		...
		try {
			Child instance = (Child) sessionFactory.getCurrentSession().get(
					"mappings_re.Child", id);
			if (instance == null) {
	...
	public List<Child> findByExample(Child instance) {
		log.debug("finding Child instance by example");
		try {
			List<Child> results = (List<Child>) sessionFactory
					.getCurrentSession().createCriteria("mappings_re.Child")
